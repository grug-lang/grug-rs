...
mod test_bindings {
    ...
    pub extern "C" fn create_entity<'a>(
        (state, _): &'a CState,
        file_id: FileId,
        err_out: &'_ mut Option<NTStrPtr<'a>>,
    ) -> Option<GrugEntityHandle<'a>> {
    ...
    pub extern "C" fn get_updated_resources(
        _state: &CState,
        count_out: &mut usize,
    ...
    pub extern "C" fn call_export_fn<'a>(
        (state, _): &CState,
        entity: GrugEntityHandle<'a>,
        fn_name: NTStrPtr<'a>,
        args: *const Value,
        args_count: usize,
    ) {
        ...
        let entity_type = Path::new(
            state
                .get_script_path_rel(entity.file_id)
    ...
    #[allow(unused_variables)]
    pub extern "C" fn dump_file_to_json<'a>(
        _state: &CState,
        input_grug_file: NTStrPtr<'a>,
    ...
    pub extern "C" fn generate_file_from_json<'a>(
        _state: &CState,
        input_json: NTStrPtr<'a>,
        output_buffer: *mut u8,
        output_buffer_len: usize,
    ) -> i32 {
    ...
    #[allow(non_camel_case_types)]
    pub type create_entity_t = for<'a> extern "C" fn(
        &'a CState,
        FileId,
        &mut Option<NTStrPtr<'a>>,
    ) -> Option<GrugEntityHandle<'a>>;
    #[allow(non_camel_case_types)]
    ...
    #[allow(non_camel_case_types)]
    pub type get_updated_resources_t =
        extern "C" fn(&CState, &mut usize) -> *const *const std::ffi::c_char;
    #[allow(non_camel_case_types)]
    pub type call_export_fn_t =
        for<'a> extern "C" fn(&'a CState, GrugEntityHandle<'a>, NTStrPtr<'_>, *const Value, usize);
    #[allow(non_camel_case_types)]
    pub type dump_file_to_json_t = extern "C" fn(&CState, NTStrPtr<'_>, *mut u8, usize) -> i32;
    #[allow(non_camel_case_types)]
    pub type generate_file_from_json_t =
        extern "C" fn(&CState, NTStrPtr<'_>, *mut u8, usize) -> i32;
    #[allow(non_camel_case_types)]
    ...
    while i < args.len() {
        if args[i] == "--continue-on-fail" {
            ...
            if i < args.len() {
                ...
                path.push('\0');
                results_json_path =
                    unsafe { Some(NTStr::from_str_unchecked(String::leak(path)).as_ntstrptr()) };
            }
        } else {
            i += 1;
        }
