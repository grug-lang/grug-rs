    ...
    pub extern "C" fn list_number_insert<'a>(
        ...
        unsafe {
            ...
            OBJECTS
                ...
                .unwrap()
                .insert(location as usize, value);
        }
        Value { void: () }
    }
...
impl<T> UnsafeStatic<T> {
    ...
    pub unsafe fn write(&mut self, value: T) {
        unsafe { self.0.as_mut_ptr().write(value) }
    }
}
...
fn main() {
    unsafe {
        ...
        state
            ...
            .unwrap();
        state
            .register_host_fn("print_number", print_number)
            .unwrap();
        state.register_host_fn("list_number", list_number).unwrap();
        state
            .register_host_fn("print_list_number", print_list_number)
            .unwrap();
        state
            .register_host_fn("list_number_insert", list_number_insert)
            .unwrap();
        state
            .register_host_fn("list_number_remove", list_number_remove)
            .unwrap();
        state
            .register_host_fn("list_number_push", list_number_push)
            .unwrap();
        state
            .register_host_fn("list_number_pop", list_number_pop)
            .unwrap();
        state
            .register_host_fn("list_number_len", list_number_len)
            .unwrap();
        state
            .register_host_fn("list_number_get", list_number_get)
            .unwrap();
        state
            .register_host_fn("list_number_set", list_number_set)
            .unwrap();
        ...
        let script_id = STATE
            .compile_grug_file("fib_script/entity-Fib.grug")
            .unwrap();
