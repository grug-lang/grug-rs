...
fn search_mod_api_path() -> Result<String> {
    ...
    loop {
        ...
        if let Ok(true) = std::fs::exists(&cwd) {
            return Ok(String::from(cwd.to_string_lossy()));
        };
        if !cwd.pop() {
            Err("Could not find mod_api in current or parent directory")?;
        }
        if !cwd.pop() {
            Err("Could not find mod_api in current or parent directory")?;
        }
...
fn print_help() {
    ...
    println!(
        "      If `-m` is not present, it will search parent directories until a 'mod_api.json' is found"
    );
}
