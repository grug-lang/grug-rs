...
fn allocate_buffer_for_file(file: &File, arena: &Arena) -> NonNull<[u8]> {
    ...
    let cap = if size > 0 {
        ((((size - 1) / 4096) + 1) * 4096) as usize
    } else {
        0
    };
    arena
        .alloc(std::alloc::Layout::from_size_align(cap, 4096).unwrap())
        .unwrap()
}
    ...
    let str = std::str::from_utf8(data).map_err(|err| {
        ...
        Error::new(
            ...
            SourceSpan {
                ...
                line: SourceSpan::get_line_from_offset(err.valid_up_to(), str),
            },
...
mod windows {
    ...
    pub fn read_files_async<'a, 'b>(
        ...
        for (iosb, file_data) in iosbs.iter_mut().zip(&mut files_data) {
            let Ok((file_text, file_path)) = file_data else {
                continue;
            };
            // Empty files will throw an EOF error, but that is expected
            if unsafe {
                !iosb.status.status.is_success() && iosb.status.status != NTSTATUS::ERR_EOF
            } {
                *file_data = Err(Error::new(
                    ...
                    "",
                    SourceSpan { offset: 0, line: 0 },
                    format_args!("IO Error (status is not Success): {:x?}", unsafe {
                        iosb.status.status
                    }),
                ));
                continue;
