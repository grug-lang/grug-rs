...
mod typed_xar {
    ...
    impl<T> Xar<T> {
        const _ASSERT_1: () = assert!(
            size_of::<XarHandle<'static, T>>() == size_of::<XarHandle<'static, Option<T>>>()
        );
        ...
        pub fn insert(&self, value: T) -> XarHandle<'_, T> {
            ...
            if let Some(free) = inner.free_list.get() {
                ...
                unsafe { XarHandle::new(chunk.add(location.1)) }
            }
        }
        ...
        fn alloc_chunk(bucket_idx: usize) -> NonNull<XarStorage<T>> {
            ...
            let layout = Layout::array::<XarStorage<T>>(chunk_size).expect("layout invalid");
    ...
    impl<T> Drop for Xar<T> {
        fn drop(&mut self) {
            ...
            for (i, chunk) in inner
                .chunks
                .iter_mut()
                .filter_map(|x| x.get_mut().as_mut())
                .enumerate()
            {
    ...
    impl<'a, T> XarHandle<'a, T> {
        ...
        pub unsafe fn detach_lifetime(self) -> XarHandle<'static, T> {
            unsafe { std::mem::transmute::<Self, XarHandle<'static, T>>(self) }
        }
    }

    impl<'a, T: std::fmt::Debug> std::fmt::Debug for XarHandle<'a, T> {
        fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
            f.debug_struct("XarHandle").field("ptr", &self.0).finish()
        }
    }
...
mod erased_xar {
    ...
    const _: () =
        assert!(size_of::<ErasedPtr<'static>>() == size_of::<Option<ErasedPtr<'static>>>());
    ...
    impl ErasedXar {
        ...
        pub fn get_slot(&self) -> ErasedPtr<'_> {
            ...
            if let Some(free) = inner.free_list.get() {
                ...
                let ret_val = unsafe { chunk.byte_add(location.1 * self.item_size()) };
                self.committed.update(|x| x + 1);
                ret_val
            }
        }
        ...
        fn chunk_layout(&self, bucket_idx: usize) -> Layout {
            Layout::from_size_align(
                self.chunk_size(bucket_idx) * self.item_size(),
                self.item_align(),
            )
        ...
        pub unsafe fn delete_with<F: FnOnce(ErasedPtr)>(&self, handle: ErasedPtr, f: F) {
            ...
            let free_list = unsafe { &(*self.inner.as_ptr()).free_list };
            unsafe {
                handle.write_value(free_list.get());
            }
        ...
        fn create_inner(mut item_layout: Layout) -> NonNull<XarInner> {
            ...
            item_layout = Layout::from_size_align(size, align).expect("invalid layout");
    ...
    mod trait_impls {
        ...
        impl<'a> std::fmt::Debug for ErasedPtr<'a> {
            fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
                f.debug_struct("ErasedPtr").field("ptr", &self.0).finish()
            }
        }
    }
