    ...
    fn serialize_expr(expr: &Expr) -> JsonValue {
        match &expr.data {
            ...
            ExprData::Call {
                ...
                if !args.is_empty() {
                    object["arguments"] =
                        args.iter().map(serialize_expr).collect::<Vec<_>>().into();
                }
                object
            }
        ...
        match statement {
            ...
            Statement::If {
                ...
                if !else_block.is_empty() {
                    object["else_statements"] = else_block
                        .iter()
                        .map(serialize_statement)
                        .collect::<Vec<_>>()
                        .into();
            ...
            Statement::While { condition, block } => object! {
...
mod de {
    ...
    fn apply_global_statement(
        ...
        if let JsonValue::Object(global_statement) = input {
            let Some(kind) =
                get_object_field(global_statement, "type", "global_statement")?.as_str()
            else {
            ...
            match kind {
                "VARIABLE_STATEMENT" => {
                    let Some(name) =
                        get_object_field(global_statement, "name", "VARIABLE_STATEMENT")?.as_str()
                    else {
                    ...
                    let ty =
                        get_object_field(global_statement, "variable_type", "VARIABLE_STATEMENT")?;

                    apply_type(ty, output)?;
                    output.push_str(" = ");

                    let assignment_expr =
                        get_object_field(global_statement, "assignment", "VARIABLE_STATEMENT")?;
                    apply_expr(assignment_expr, output)?;
                    Ok(())
                }
                "EXPORT_FN" => {
                    let Some(name) =
                        get_object_field(global_statement, "name", "EXPORT_FN")?.as_str()
                    else {
                    ...
                    if let Ok(parameters) =
                        get_object_field(global_statement, "parameters", "EXPORT_FN")
                    {
                    ...
                    let body_statements =
                        get_object_field(global_statement, "statements", "EXPORT_FN")?;
                    apply_statements(body_statements, indentation + 1, output)?;
                    Ok(())
                }
                "LOCAL_FN" => {
                    let Some(name) =
                        get_object_field(global_statement, "name", "LOCAL_FN")?.as_str()
                    else {
                    ...
                    output.push_str("(");
                    if let Ok(parameters) =
                        get_object_field(global_statement, "parameters", "LOCAL_FN")
                    {
                        apply_parameters(parameters, output)?;
                    }
                    output.push_str(") ");
                    ...
                    let body_statements =
                        get_object_field(global_statement, "statements", "LOCAL_FN")?;
                ...
                "COMMENT_STATEMENT" => {
                    let Some(value) =
                        get_object_field(global_statement, "comment", "COMMENT_STATEMENT")?
                            .as_str()
                    else {
        ...
        for statement in statements {
            ...
            match kind {
                "VARIABLE_STATEMENT" => {
                    ...
                    let Some(name) =
                        get_object_field(statement, "name", "VARIABLE_STATEMENT")?.as_str()
                    else {
                    ...
                    if let Ok(ty) =
                        get_object_field(statement, "variable_type", "VARIABLE_STATEMENT")
                    {
                        output.push_str(": ");
                        apply_type(ty, output)?;
                    }
                ...
                "CALL_STATEMENT" => {
                    ...
                    let Some(name) =
                        get_object_field(statement, "name", "CALL_STATEMENT")?.as_str()
                    else {
                    ...
                    if let Ok(arguments) =
                        get_object_field(statement, "arguments", "CALL_STATEMENT")
                    {
                ...
                "IF_STATEMENT" => {
                    ...
                    'outer: loop {
                        output.push_str("if ");
                        apply_expr(
                            get_object_field(statement, "condition", "IF_STATEMENT")?,
                            output,
                        )?;
                        output.push_str(" ");

                        if let Ok(if_statements) =
                            get_object_field(statement, "if_statements", "IF_STATEMENT")
                        {
                            apply_statements(if_statements, indentation + 1, output)?;
                        } else {
                            apply_statements(&JsonValue::Array(vec![]), indentation + 1, output)?;
                        }
                        if let Ok(else_block) =
                            get_object_field(statement, "else_statements", "IF_STATEMENT")
                        {
                            ...
                            'chained: {
                                if let &[JsonValue::Object(ref first_statement)] =
                                    else_block.as_slice()
                                {
                                    let Some(kind) =
                                        get_object_field(first_statement, "type", "statement")?
                                            .as_str()
                                    else {
                                        return Err(JsonDeserializeError::StatementKindNotString);
                                    };
                ...
                "WHILE_STATEMENT" => {
                    ...
                    apply_expr(
                        get_object_field(statement, "condition", "WHILE_STATEMENT")?,
                        output,
                    )?;
                    output.push_str(" ");
                    apply_statements(
                        get_object_field(statement, "statements", "WHILE_STATEMENT")?,
                        indentation + 1,
                        output,
                    )?;
                }
                ...
                "COMMENT_STATEMENT" => {
                    apply_indentation(indentation, output);
                    let Some(value) =
                        get_object_field(statement, "comment", "COMMENT_STATEMENT")?.as_str()
                    else {
                        return Err(JsonDeserializeError::CommentValueNotString);
                    };
                    output.push_str("# ");
    ...
    fn apply_expr(input: &JsonValue, output: &mut String) -> Result<(), JsonDeserializeError> {
        ...
        match ty {
            "STRING_EXPR" => {
                let Some(value) = get_object_field(input, "str", "LITERAL_EXPRESSION")?.as_str()
                else {
            ...
            "IDENTIFIER_EXPR" => {
                let Some(value) = get_object_field(input, "str", "IDENTIFIER_EXPR")?.as_str()
                else {
                    return Err(JsonDeserializeError::LiteralExpressionValueNotString);
                };
                output.push_str(value);
            ...
            "NUMBER_EXPR" => {
                let Some(string) = get_object_field(input, "value", "LITERAL_EXPRESSION")?.as_str()
                else {
    ...
    fn get_object_field<'a>(
        ...
        field: &'static str,
        parent_context: &str,
    ) -> Result<&'a JsonValue, JsonDeserializeError> {
        match &input[field] {
