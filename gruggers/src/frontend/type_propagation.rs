			...
			ExprData::Call{
				...
				if let Some(type_name) = static_type_name {
					...
					let Some((_, host_fn)) = static_methods.iter().find(|(fn_name, _)| fn_name.as_str() == name) else {
						if self.mod_api.classes().get(type_name).is_some_and(|class| class.methods.iter().any(|(fn_name, _)| fn_name.as_str() == name)) {
							return Err(self.new_error(
								*name_span,
					...
					// A static method has no receiver to infer the owner's
					// generics from, so every generic it uses is inferred
					// from its arguments and return type, exactly like a
					// free host function's.
					let generics = if let Some(substitutions) = substitutions {
						// for the second time through, replace the existentials as they are created
						arena.slice_from_iter(host_fn.generics.iter().map(|_| {
							let Type::Existential{idx} = ty_ctx.create_existential(name, *name_span) else {unreachable!()};
					...
					if substitutions.is_some() {
						...
						// generic functions need to call the registerer
							...
							if let Some(fn_registerer) = host_fn.registerer {
								...
								panic!("generic static method {}.{} was not registered (Note: This error is not triggerred by grug_tests)", type_name, name);
							}
						}
					}
					Self::convert_mod_api_type(host_fn.return_ty, generics, arena)
					...
					let Some((_, host_fn)) = class.methods.iter().find(|(fn_name, _)| fn_name.as_str() == name) else {
						if let Some(_) = class.get_static_method(name) {
							return Err(self.new_error(
								*name_span,
