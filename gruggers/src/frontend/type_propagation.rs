...
use crate::ast::{
    BinaryOperator, Expr, ExprData, HelperFunction, Parameter, Statement, Type, UnaryOperator,
};
...
impl<'mod_api: 'arena, 'arena: 'temp, 'temp> TypePropagator<'mod_api, 'arena, 'temp> {
    ...
    pub fn fill_result_types(
        ...
        type_propagator.global_variables.insert(
            nt!("me"),
            Type::Id {
                name: type_propagator
                    .arena
                    .copy_str_into_nt(entity_type)
                    .as_ntstrptr(),
                generics: &[],
            },
        );

        let variables = ast.global_statements.iter_mut().filter_map(|st| match st {
            GlobalStatement::Variable(x) => Some(x),
            _ => None,
        });
        ...
        let mut on_functions = ast
            .global_statements
            .iter_mut()
            .filter_map(|st| match st {
                GlobalStatement::OnFunction(x) => Some(x),
                _ => None,
            })
            .collect::<Vec<_>>();
        for (on_fn_name, mod_api_on_fn) in type_propagator.entity.export_fns.iter() {
            let Some((current_index, current_on_fn)) = on_functions
                .iter_mut()
                .enumerate()
                .find(|(_, on_fn)| on_fn.name.to_ntstr() == &**on_fn_name)
            else {
                continue;
            };
            ...
            for (param, arg) in mod_api_on_fn
                .parameters
                .iter()
            ...
            if let Some(class) = self.mod_api.classes().get(name.to_str()) {
                if generics.len() != class.generics.len() {
                    return Err(self.new_error(
                        err_span,
                        format_args!(
                            "type {} has {} generics, but was given {}",
                            name,
                            class.generics.len(),
                            generics.len()
                        ),
            ...
            ExprData::Call {
                ...
                if fn_name.starts_with("_") {
                    return Err(self.new_error(
                        assignment_expr.span,
                        format_args!(
                            "The global variable '{}' isn't allowed to call local functions",
                            name
                        ),
                    ));
                }
    ...
    fn convert_mod_api_type<'a>(
        mod_api_type: Type<'mod_api>,
        replacements: &[Type<'a>],
        arena: &'a Arena,
    ) -> Type<'a>
    where
        ...
        'arena: 'a,
    {
        let result_ty = match &mut assignment_expr.data {
            ...
            ExprData::Number { .. } => Type::Number,
            ExprData::Unary { op, expr, op_span } => {
                if let Expr {
                    data: ExprData::Unary { op: next_op, .. },
                    ..
                } = expr
                    && next_op == op
                {
                ...
                ty_ctx
                    .add_constraint(*op_span, expected, result_ty)
                    .map_err(|err| {
                        self.new_error(
                            err.span,
                            format_args!(
                                "Found '{}' before {}, but it can only be put before a {}",
                                op,
                                err.diff.swapped(),
                                expected
                            ),
            ...
            ExprData::Binary {
                ...
                let result_0 = if let Some(ty) = ty_ctx.get_current_type(result_0) {
                    ty
                } else {
                    result_0
                };
                let result_1 = if let Some(ty) = ty_ctx.get_current_type(result_1) {
                    ty
                } else {
                    result_1
                };
                ...
                match (&result_0, &result_1, *op) {
                    ...
                    (Type::String, Type::String, BinaryOperator::Plus) => {
                        return Err(
                            self.new_error(*op_span, format_args!("cannot add strings with '+'"))
                        );
                    }
                ...
                let (expected_type, result_type) = match op {
                    ...
                    | BinaryOperator::Less
                    | BinaryOperator::LessEquals => (Type::Number, Type::Bool),
                    BinaryOperator::Plus
                    | BinaryOperator::Minus
                    | BinaryOperator::Multiply
                    | BinaryOperator::Division => (Type::Number, Type::Number),
                };
                ...
                for (expr_result, expr_span) in [(result_0, left.span), (result_1, right.span)] {
                    ty_ctx
                        .add_constraint(expr_span, expected_type, expr_result)
                        .map_err(|err| {
                            self.new_error(
                                *op_span,
                                format_args!(
                                    "'{}' operator expects {} but got {}",
                                    op,
                                    expected_type,
                                    err.diff.swapped()
                                ),
            ...
            ExprData::Call {
				...
				'outer: {
					...
					let host_fn = if let Some(receiver) = receiver_slot {
						...
						if let Expr {
							data: ExprData::Identifier(recv_name),
							..
						} = receiver
							...
							&& let Some(static_methods) = self.mod_api.static_methods_of(recv_name)
						{
							...
							let actual_receiver_ty = if let Some(ty) = ty_ctx.get_current_type(receiver_type)
							{
								ty
							} else {
								return Err(self.new_error(
					...
					let parameters = arena.slice_from_iter(host_fn.parameters.iter().map(|param| Parameter {
						ty: Self::convert_mod_api_type(param.ty, generics, arena),
						..*param
					}));

					// add a constraint between the expected type of the
					// receiver and the actual type of the receiver
					if let Some((actual_receiver_ty, mod_api_receiver_ty)) = receiver_info {
    ...
    fn validate_and_fix_resource_string<'a>(
        &mut self,
        value: &str,
        extension: &str,
        span: SourceSpan,
        ...
        if value.is_empty() {
            return Err(self.new_error(span, format_args!("Resources can't be empty strings")));
        ...
        string.push(value);
        let resource_str = Box::leak(NTStr::box_from_str_in(
    ...
    fn validate_and_fix_entity_string<'a>(
        &mut self,
        entity_string_old: &mut NTStrPtr<'a>,
        span: SourceSpan,
        arena: &'a Arena,
    ) -> Result<(), Error> {
        ...
        if entity_string.is_empty() {
            return Err(self.new_error(span, format_args!("Entities can't be empty strings")));
        }

        let (mod_name, entity_name) =
            if let Some((mod_name, entity_name)) = entity_string.split_once(":") {
                ...
                if mod_name == self.current_mod_name {
                    return Err(self.new_error(
                        span,
                        format_args!(
                            "Entity string ('{}') cannot refer to its own mod",
                            entity_string
                        ),
                    ));
                }
        ...
        if let Some(ch) = mod_name.chars().find(|ch| {
        ...
        if let Some(ch) = entity_name.chars().find(|ch| {
            !(ch.is_ascii_lowercase() || ch.is_ascii_digit() || *ch == '_' || *ch == '-')
        }) {
            return Err(self.new_error(
                span,
        ...
        if mod_name.is_empty() {
            *entity_string_old = Box::leak(NTStr::box_from_str_in(
                &format!("{}:{}", self.current_mod_name.display(), entity_name),
                arena,
            ))
            .as_ntstrptr()
        }
    ...
    fn add_local_variable(
        ...
        if self.get_global_variable_type(name).is_some() {
            return Err(self.new_error(
                name_span,
                format_args!(
                    "The local variable '{}' shadows an earlier global variable",
                    name
                ),
            ));
        }
        if self.get_local_variable_type(name).is_some() {
            return Err(self.new_error(
                name_span,
                format_args!(
                    "The local variable '{}' shadows an earlier local variable",
                    name
                ),
            ));
        }
        ...
        match self.global_variables.entry(name) {
            Entry::Occupied(_) => {
                return Err(self.new_error(
                    name_span,
                    format_args!(
                        "The global variable '{}' shadows an earlier global variable",
                        name
                    ),
                ));
            }
...
struct TyCtx<'a, 'err> {
    ...
    existentials: Vec<ExistentialData<'a>, &'a Arena>,
...
impl<'a, 'err> TyCtx<'a, 'err> {
    ...
    fn get_current_type(&self, ty: Type<'a>) -> Option<Type<'a>> {
        fn get_current_type_inner<'a>(
            substitutions: &[Type<'a>],
            stack: StackLL<usize>,
        ) -> Option<Type<'a>> {
        ...
        match ty {
            Type::Existential { idx } => get_current_type_inner(
                &self.substitutions,
                StackLL {
                    current: idx,
                    parent: None,
    ...
    fn create_existential(
        ...
        self.substitutions.push(Type::Existential{ idx: new_existential });
        new_existential
    }
    ...
    fn add_constraint(
        ...
        while let Some(constraint) = self.constraints.pop() {
            match constraint {
                ...
                (_, Type::Resource { .. }) | (Type::Resource { .. }, _) => {
                    unreachable!("resource strings never appear in generics")
                }
                (_, Type::Entity { .. }) | (Type::Entity { .. }, _) => {
                    unreachable!("entity strings never appear in generics")
                }
                (
                    Type::Id {
                        name: left_name, ..
                ...
                (
                    ...
                    assert_eq!(
                        left_generics.len(),
                        right_generics.len(),
                        "You forgot to verify the number of generics on types"
                    );
            ...
            match (ty, imp_ty) {
                (
                    ...
                    ty_generics
                        .iter()
                        .zip(imp_generics)
                        .all(|(ty, imp)| type_matches_implementor(*ty, *imp, generics))
                }
                (ty, Type::Existential { idx }) => generics
                    .get(idx)
                ...
                (Type::Resource { .. }, _) | (_, Type::Resource { .. }) => {
                    unreachable!("resource strings cannot be used in generics")
                }
                (Type::Entity { .. }, _) | (_, Type::Entity { .. }) => {
                    unreachable!("entity strings cannot be used in generics")
                }
                _ => false,
            }
        }
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        fn get_type_name(ty: Type<'_>) -> &str {
            match ty {
                ...
                Type::Number => "number",
                Type::String => "string",
                Type::Id { name, generics: _ } => name.to_str(),
        ...
        match (self.print, self.diff) {
            (
                Type::Id {
                    name: name_left,
                    generics: generics_left,
                },
                Type::Id {
                    name: name_right,
                    generics: generics_right,
                },
            ) if name_left == name_right => {
                f.write_str("[")?;
                for (i, (print, diff)) in generics_left
                    .iter()
                    .copied()
                    .zip(generics_right.iter().copied())
                    .enumerate()
                {
                    Self { print, diff }.fmt(f)?;
