            ...
            ExprData::Call {
                ...
                let static_type_name: Option<&str> = match receiver_slot.as_deref() {
                    Some(Expr {
                        ...
                        if self.get_variable_type(recv_name).is_none()
                            && self.mod_api.declares_type(recv_name)
                        {
                            Some(recv_name)
                        } else {
                            None
                ...
                if let Some(type_name) = static_type_name {
                    ...
                    let static_methods = self
                        .mod_api
                        .static_methods_of(type_name)
                    ...
                    let Some((_, host_fn)) = static_methods
                        ...
                        let is_method =
                            self.mod_api.classes().get(type_name).is_some_and(|class| {
                                class
                                    .methods
                                    .iter()
                    ...
                    let receiver_type = if let Some(ty) = ty_ctx.get_current_type(receiver_type) {
                        ty
                    } else {
                        return Err(self.new_error(
                            receiver.span,
                    ...
                    let (receiver_ty, receiver_methods) =
                        if let Some(class) = self.mod_api.classes().get(receiver_name) {
...
impl<'a, 'err> TyCtx<'a, 'err> {
    ...
    fn create_existential(
        ...
        let new_existential = Type::Existential {
            idx: self.existentials.len(),
        };
