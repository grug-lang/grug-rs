...
impl<'a> PartialEq for TypeWrapper<'a> {
    fn eq(&self, other: &Self) -> bool {
        ...
        match (self.0, other.0) {
            ...
            ) => name_1.ptr_eq(name_2) && std::ptr::eq(generics_1, generics_2),
            (
                Resource {
...
static GLOBAL_TYPE_STORAGE: LazyLock<RwLock<TypeStorage>> =
    LazyLock::new(|| RwLock::new(TypeStorage::new()));
...
impl TypeStorage {
    ...
    pub fn insert_type(&mut self, ty: Type) -> Type<'static> {
        ...
        match ty {
            ...
            Existential { idx: usize } => Existential { idx: usize },
        }
    }
        ...
        match self.type_lists.get::<&[TypeWrapper]>(&list) {
            Some(list) => {
                ...
                let list: &'static [TypeWrapper<'static>] = unsafe {
                    std::mem::transmute::<&'a [TypeWrapper<'b>], &'static [TypeWrapper<'static>]>(
                        list,
                    )
                };
            ...
            None => {
                ...
                }
                {
                    let list = list
                        .iter()
                        .map(|ty| TypeWrapper(self.insert_type(ty.0)))
                        .collect::<Vec<_>>();
