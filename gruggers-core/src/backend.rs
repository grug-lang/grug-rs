...
pub unsafe trait Backend {
	...
	#[must_use]
	fn init_entity(&self, state: &Self::GrugState, entity: &GrugEntity) -> bool;
	...
	#[must_use]
	unsafe fn call_on_function_raw(&self, state: &Self::GrugState, entity: &GrugEntity, on_fn_index: usize, values: *const GrugValue) -> bool;
	/// Run the on function at index `on_fn_index` of the script associated
	/// with `entity`.
	///
	...
	#[must_use]
	fn call_on_function(&self, state: &Self::GrugState, entity: &GrugEntity, on_fn_index: usize, values: &[GrugValue]) -> bool;
}
...
impl<T: Backend<GrugState=GrugState>, GrugState: State> From<T> for ErasedBackend<GrugState> {
	fn from(other: T) -> Self {
		#[allow(improper_ctypes_definitions)]
		extern "C" fn insert_file<T: Backend<GrugState=GrugState>, GrugState: State>(data: NonNull<()>, id: GrugScriptId, file: GrugAst<'_>) {
			T::insert_file(
		...
		extern "C" fn init_entity<T: Backend<GrugState=GrugState>, GrugState: State>(data: NonNull<()>, state: &GrugState, entity: &GrugEntity) -> bool {
			T::init_entity(
				unsafe{data.cast::<T>().as_ref()},
				state, 
				entity
		...
		extern "C" fn clear_entities<T: Backend<GrugState=GrugState>, GrugState: State>(data: NonNull<()>) {
			T::clear_entities(
				unsafe{data.cast::<T>().as_mut()},
			)
		}

		extern "C" fn destroy_entity_data<T: Backend<GrugState=GrugState>, GrugState: State>(data: NonNull<()>, entity: &GrugEntity) -> bool {
		...
		/// SAFETY: `values` must point to a buffer of at least as many values as on_fn_id expects
		unsafe extern "C" fn call_on_function_raw<T: Backend<GrugState=GrugState>, GrugState: State>(data: NonNull<()>, state: &GrugState, entity: &GrugEntity, on_fn_index: usize, values: *const GrugValue) -> bool {
		...
		fn call_on_function<T: Backend<GrugState=GrugState>, GrugState: State>(data: NonNull<()>, state: &GrugState, entity: &GrugEntity, on_fn_index: usize, values: &[GrugValue]) -> bool {
			T::call_on_function(
				unsafe{data.cast::<T>().as_ref()},
				state, 
				entity,
		...
		// destroys the resources owned by the backend
		extern "C" fn drop<T: Backend<GrugState=GrugState>, GrugState: State>(data: NonNull<()>) {
...
// 	state: NonNull<()>,
// 	handler: for<'a> extern "C" fn (NonNull<()>, u32, Option<NTStrPtr<'a>>),
// }
...
// 			RuntimeError::ExceededTimeLimit => self.handler(self.data, error.code(), None),
// 			RuntimeError::GameFunctionError{message} => {
// 				let string;
// 				let message = NTStr::from_str(message).unwrap_or_else(|| {
// 					string = format!("{}\n", message);
// 					NTStr::from_str(&string).unwrap()
// 				});
// 				self.handler(self.data, error.code(), Some(message.as_ntstrptr()));
// 			}
// 		}
// 	}
// }

// struct CBackend<B: Backend<GrugState=CStateWithHandler>> {
// 	runtimer_error_handler: for<'a> extern "C" fn (NonNull<()>, u32, Option<NTStrPtr<'a>>),
// 	backend: B,
// }

// impl<B: Backend<GrugState=CStateWithHandler>> From<CBackend<B>> for ErasedBackend {
// 	fn from(other: CBackend) -> Self {
// 		#[allow(improper_ctypes_definitions)]
// 		extern "C" fn insert_file(data: NonNull<()>, id: GrugScriptId, file: GrugAst<'_>) {
// 			T::insert_file(
// 				unsafe{data.cast::<T>().as_ref()},
// 				id,
// 				file
...
// 		extern "C" fn init_entity(data: NonNull<()>, state: &GrugState, entity: &GrugEntity) -> bool {
// 			T::init_entity(
// 				unsafe{data.cast::<T>().as_ref()},
// 				state, 
// 				entity
...
// 		extern "C" fn clear_entities(data: NonNull<()>) {
// 			T::clear_entities(
// 				unsafe{data.cast::<T>().as_mut()},
// 			)
// 		}

// 		extern "C" fn destroy_entity_data(data: NonNull<()>, entity: &GrugEntity) -> bool {
// 			T::destroy_entity_data(
// 				unsafe{data.cast::<T>().as_ref()},
// 				entity
// 			)
// 		}
// 		/// SAFETY: `values` must point to a buffer of at least as many values as on_fn_id expects
// 		unsafe extern "C" fn call_on_function_raw(data: NonNull<()>, state: &GrugState, entity: &GrugEntity, on_fn_index: usize, values: *const GrugValue) -> bool {
// 			unsafe{T::call_on_function_raw(
// 				data.cast::<T>().as_ref(),
// 				state, 
// 				entity,
...
// 			)}
// 		}
// 		fn call_on_function(data: NonNull<()>, state: &GrugState, entity: &GrugEntity, on_fn_index: usize, values: &[GrugValue]) -> bool {
// 			T::call_on_function(
// 				unsafe{data.cast::<T>().as_ref()},
// 				state, 
// 				entity,
