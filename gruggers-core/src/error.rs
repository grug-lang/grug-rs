...
impl SourceSpan {
    ...
    pub fn get_col(self, text: &str) -> usize {
        ...
        for (i, ch) in text
            .get(..self.offset)
            .expect("span within source code bounds")
            .iter()
            .rev()
            .enumerate()
        {
...
impl<'a> GrugError<'a> {
    ...
    pub fn new_error_in<A: Allocator>(
        ...
        source_text: &str,
        err_span: SourceSpan,
        error_message: std::fmt::Arguments,
        alloc: &'a A,
    ) -> Self {
        ...
        if error_kind.matches(&ErrorKind::FILE_NAME_ERROR) {
            ...
            || error_kind.matches(&ErrorKind::UNEXPECTED_NULL_BYTE)
        {
            write!(
                err_string,
                "  in ({}:{line}:{column})\n\
        ...
        let function_name = unsafe {
            Box::leak(NTStr::box_from_str_in(function_name, &alloc))
                .as_ntstrptr()
                .detach_lifetime()
        };
        ...
        let source_line = {
            ...
            unsafe {
                slice
                    .as_mut_ptr()
                    .cast::<u8>()
                    .copy_from(source_line.as_ptr(), source_line.len())
            };
        ...
        let error_string = unsafe {
            NTStr::from_str_unchecked(str::from_utf8_unchecked(err_string.leak()))
                .as_ntstrptr()
                .detach_lifetime()
        };

        let mut err_message = Vec::new_in(&alloc);
        write!(err_message, "{}\0", error_message).expect("writing into a vec should never fail");

        // SAFETY: err_string only contains utf8 strings, and is null terminated
        // The string can last as long as the allocator exists, and we never give
        // out a 'static pointer to the string
        let error_message = unsafe {
            NTStr::from_str_unchecked(str::from_utf8_unchecked(err_message.leak()))
                .as_ntstrptr()
                .detach_lifetime()
        };
