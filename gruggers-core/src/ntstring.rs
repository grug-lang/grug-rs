...
mod bytes {
    ...
    const _: () =
        const { assert!(std::mem::size_of::<NTBytes>() == std::mem::size_of::<Option<NTBytes>>()) };
    ...
    impl<'a> NTBytes<'a> {
        ...
        pub fn from_bytes(value: &'a [u8]) -> Option<Self> {
            ...
            if value[..(value.len() - 1)].contains(&b'\0') {
                return None;
            }
    ...
    pub fn copy_box_nt_bytes_in<A: Allocator>(bytes: &[u8], a: A) -> Box<[u8], A> {
        ...
        let (ptr, a) =
            Box::into_raw_with_allocator(Box::<[u8], _>::new_uninit_slice_in(bytes.len() + 1, a));
