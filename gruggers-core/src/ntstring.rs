...
mod osstr {
	...
	impl NTOsStr {
		...
		pub fn as_osstr(&self) -> &OsStr {
			...
			unsafe{OsStr::from_encoded_bytes_unchecked(&self.0.as_encoded_bytes()[..self.0.len() - 1])}
		}
		...
		pub fn try_from_osstr(value: &OsStr) -> Result<&NTOsStr, NTStrError> {
			if let Some(last) = value.as_encoded_bytes().last() && *last == b'\0' {
				for (i, byte) in value.as_encoded_bytes()[0..value.len()-1].iter().enumerate() {
					if *byte == b'\0' {
						return Err(NTStrError::UnexpectedNullByte{location: i});
					}
				}
	...
	const _: () = const {assert!(std::mem::size_of::<NTOsStrPtr>() ==  std::mem::size_of::<Option<NTOsStrPtr>>())};
